...
Processr.prototype.enhance = function( stream ){
  ...
  var filepath = 'cv_'+ self.data.hash + '.gif';
  filepath = require('path').join(tmpDir, filepath);
  ...
  function onUpload(err, res){
    ...
    console.log('Cover Image Url: %s', res.req.url);
  ...
  console.log('Preparing to write file: %s', this.data.filename);
  ...
  gm(stream, this.data.filename + '[0]')
    .write( filepath, function(err){
      ...
      console.log('Cover image created: %s', path);

      s3.putFile( filepath, 'cv_'+self.data.filename, {
        'x-amz-acl': 'public-read'
      }, onUpload);
    });
